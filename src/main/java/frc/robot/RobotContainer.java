...
public class RobotContainer {
  ...
  private SwerveDrivetrain m_swerveDrivetrain = new SwerveDrivetrain(m_hardwareMap.swerveHardware);
  private SwerveJoystickCommand m_swerveJoystickCommand = new SwerveJoystickCommand(m_swerveDrivetrain,
      m_hardwareMap.driveController);
  private IntakeSubsystem m_intakeSubsystem = new IntakeSubsystem(m_hardwareMap.intakeHardware);
  private MoveArmCommand m_moveArmCommand = new MoveArmCommand(m_hardwareMap.opperatorController, m_intakeSubsystem);
  ...
  public RobotContainer() {
    ...
    m_swerveDrivetrain.setDefaultCommand(m_swerveJoystickCommand);
  }
  ...
  private void configureButtonBindings() {
    ...
    new JoystickButton(m_hardwareMap.opperatorController, Button.kBumperLeft.value)
        .whenPressed(new ShooterOnCommand(m_shooterSubsystem));
    // turns off shooter when Right Bumper is pressed
    new JoystickButton(m_hardwareMap.opperatorController, Button.kBumperRight.value)
        .whenPressed(new ShooterOffCommand(m_shooterSubsystem));
    // runs intake forwards while X is held
    new JoystickButton(m_hardwareMap.opperatorController, Button.kX.value)
    ...
    new JoystickButton(m_hardwareMap.opperatorController, Button.kY.value)
        .whenHeld(new IntakeCommand(m_intakeSubsystem));
    // runs the shoot one ball command while A is held
    new JoystickButton(m_hardwareMap.opperatorController, Button.kA.value)
        .whenHeld(new ShootOneBallCommand(m_shooterSubsystem));
